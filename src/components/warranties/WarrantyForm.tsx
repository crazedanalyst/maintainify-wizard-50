...
const categoryOptions: Category[] = ['Home', 'Electronics', 'Plumbing', 'Electrical', 'HVAC', 'Appliances', 'Outdoor', 'Vehicles', 'Other'];
const WarrantyForm = ({
  warranty,
  onSave,
  onCancel
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const {
          data,
          error
        } = await supabase.functions.invoke('warranty-ocr', {
          body: {
        ...
        if (data) {
          ...
          toast({
            ...
            description: data.success ? 'Information has been extracted from the document and applied to the form.' : 'Could only extract limited information from the document.'
          });
        } else {
  ...
  const simulateOcrExtraction = () => {
    ...
    const purchaseDateIsDefault = purchaseDate && purchaseDate.getDate() === today.getDate() && purchaseDate.getMonth() === today.getMonth() && purchaseDate.getFullYear() === today.getFullYear();
    ...
    const expiryDateIsDefault = expiryDate && expiryDate.getDate() === today.getDate() && expiryDate.getMonth() === today.getMonth() && expiryDate.getFullYear() === today.getFullYear();
  ...
  return <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <Input value={itemName} onChange={e => setItemName(e.target.value)} placeholder="e.g. Refrigerator, TV, Roof" required className="h-8 text-xs mt-1" />
        </div>
        
        <div>
          <label className="text-xs font-medium">Manufacturer*</label>
          <Input value={manufacturer} onChange={e => setManufacturer(e.target.value)} placeholder="e.g. Samsung, LG, CertainTeed" required className="h-8 text-xs mt-1" />
        </div>
      </div>
      ...
      <div>
        ...
        <Select value={category} onValueChange={value => setCategory(value as Category)} required>
          ...
          <SelectContent>
            {categoryOptions.map(option => <SelectItem key={option} value={option} className="text-xs">{option}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal text-xs h-8 mt-1", !purchaseDate && "text-muted-foreground")}>
                <CalendarIcon className="mr-1 h-3 w-3" />
                {purchaseDate ? formatDate(purchaseDate) : <span>Pick a date</span>}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="start">
              <Calendar mode="single" selected={purchaseDate} onSelect={setPurchaseDate} initialFocus className={cn("p-3 pointer-events-auto")} />
            </PopoverContent>
          </Popover>
        </div>
        ...
        <div>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className={cn("w-full justify-start text-left font-normal text-xs h-8 mt-1", !expiryDate && "text-muted-foreground")}>
                <CalendarIcon className="mr-1 h-3 w-3" />
      ...
      <div>
        ...
        <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Add details about the warranty coverage..." className="min-h-[60px] text-xs resize-none mt-1" />
      </div>
      
      <div>
        <label className="text-xs font-medium">Scan Warranty Card to Extract Details</label>
        <div className="mt-1 flex items-center space-x-2">
          <Input ref={fileInputRef} type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={handleFileUpload} className="text-xs h-8" />
          <Button type="button" size="sm" variant="secondary" className="h-8 text-xs" onClick={() => fileInputRef.current?.click()}>
            <Upload className="h-3.5 w-3.5 mr-1" />
            Browse
          </Button>
        </div>
        
        {isProcessingOCR && <div className="mt-2 flex items-center space-x-2 text-xs text-blue-600">
            <Loader2 className="h-3.5 w-3.5 animate-spin" />
            <span>Processing document with OCR...</span>
          </div>}
        
        {ocrRawText && <div className="mt-2 p-2 bg-gray-50 rounded-md">
        ...
        {documentFile && <div className="mt-2 flex items-center justify-between bg-blue-50 p-2 rounded-md">
            ...
            <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 rounded-full text-gray-400 hover:text-red-500" onClick={() => setDocumentFile(null)}>
              ×
            </Button>
          </div>}
        
        {documents.length > 0 && <div className="mt-2 space-y-2">
            {documents.map((doc, index) => <div key={index} className="flex items-center justify-between bg-gray-50 p-2 rounded-md">
                <div className="flex items-center space-x-2">
                ...
                <Button type="button" variant="ghost" size="sm" className="h-6 w-6 p-0 rounded-full text-gray-400 hover:text-red-500" onClick={() => handleRemoveDocument(index)}>
                  ×
                </Button>
              </div>)}
          </div>}
      ...
      <div className="flex justify-end space-x-2 pt-2">
        {onCancel && <Button type="button" variant="outline" size="sm" onClick={onCancel} className="h-8 text-xs">
            Cancel
          </Button>}
